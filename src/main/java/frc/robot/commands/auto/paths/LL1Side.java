...
public class LL1Side extends SequentialCommandGroup {
  /** Creates a new L2Side. */
  public LL1Side(SwerveSubsystem m_SwerveSubsystem, CoralSubsystem m_CoralSubsystem, ElevatorSubsystem m_ElevatorSubsystem) {
  ...
  addCommands(new ParallelDeadlineGroup(new WaitCommand(2.7), new TurnWheelsAndDrive(m_SwerveSubsystem, 0, 2.7, 0)), new ParallelDeadlineGroup(new WaitCommand(1.5), new TurnToAngle(m_SwerveSubsystem, 60), new SetCoralState(m_CoralSubsystem, CoralStates.kL1), new SetElevatorState(m_ElevatorSubsystem, ElevatorStates.kCL1)), new ParallelDeadlineGroup(new WaitCommand(2.7), new TurnWheelsAndDrive(m_SwerveSubsystem, 1, 0, 60)), new ParallelDeadlineGroup(new WaitCommand(1), new IntakeCoral(m_CoralSubsystem, 5)), new ParallelDeadlineGroup(new WaitCommand(1.5), new SetCoralState(m_CoralSubsystem, CoralStates.kRest), new SetElevatorState(m_ElevatorSubsystem, ElevatorStates.kRest)));
  }
}
